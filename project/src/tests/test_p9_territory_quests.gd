...
func test_resident_stage_starts_novice():
	_reset()
	TerritorySystem.residents.append(
		{
			"name": "测试1",
			"job": "lumberjack",
			"assigned_building": "lumber_mill",
			"sorties_worked": 0
		}
	)
...
func test_settle_sortie_increments_worker():
	...
	# 闲置居民不该累加
	TerritorySystem.residents.append(
		{"name": "闲", "job": "idle", "assigned_building": "", "sorties_worked": 0}
	)
...
func test_merchant_refresh_on_sortie():
	...
	for i in 3:
		TerritorySystem.settle_sortie()
	_check(
		"merchant: stock filled",
		TerritorySystem.merchant_stock.size() > 0,
		"got " + str(TerritorySystem.merchant_stock.size())
	)
...
func test_merchant_buy_material():
	...
	_check(
		"merchant: material added",
		GameState.get_material(mat_item["material_id"]) == before_mat + int(mat_item["amount"]),
		""
	)
	_check(
		"merchant: gold deducted",
		GameState.total_gold == before_gold - int(mat_item["cost_gold"]),
		""
	)
...
func test_quest_refresh():
	...
	_check(
		"quest: filled to max",
		QuestSystem.active_quests.size() == QuestSystem._max_active(),
		"got " + str(QuestSystem.active_quests.size())
	)
...
func test_quest_kill_tag():
	...
	_check(
		"quest: completed",
		QuestSystem.active_quests.is_empty(),
		"still active: " + str(QuestSystem.active_quests)
	)
...
func test_quest_kill_boss():
	...
	_check(
		"quest: normal does not progress boss", QuestSystem.active_quests[0]["progress"] == 0, ""
	)
	# Boss 计
...
func test_quest_drop_rarity():
	...
	QuestSystem.active_quests = [
		{
			"id": "qdrop",
			"template_id": "quest_drop_legendary",
			"type": "drop_rarity",
			"rarity": "legendary",
			"target": 2,
			"progress": 0,
...
func test_quest_clear_dungeon():
	...
	QuestSystem.active_quests = [
		{
			"id": "qclear",
			"template_id": "quest_clear_dungeon",
			"type": "clear_dungeon",
			"target": 1,
			"progress": 0,
			"rewards": {"gold": 80}
