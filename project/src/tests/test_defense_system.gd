...
func test_trigger_interval():
	...
	for i in 4:
		...
		_check("trigger: sortie %d not trigger" % (i+1),
...
func test_victory_reward():
	...
	_check("victory: timber added",
		GameState.get_material("timber") > timber_before,
		"got " + str(GameState.get_material("timber")))
	_check("victory: result recorded",
		TerritorySystem.last_defense_result == "victory", "")
...
func test_defeat_penalty():
	...
	_check("defeat: result recorded",
		TerritorySystem.last_defense_result == "defeat", "")
