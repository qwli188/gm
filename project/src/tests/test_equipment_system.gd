...
func _make_weapon(dmg: float, set_id: String = "", affixes: Array = []) -> Dictionary:
	return {
		"id": "test_weapon", "slot": "weapon", "display_name": "测试武器",
		"rarity": "rare", "set_id": set_id,
...
func _make_helmet(armor: float, set_id: String = "", affixes: Array = []) -> Dictionary:
	return {
		"id": "test_helmet", "slot": "helmet", "display_name": "测试头盔",
		"rarity": "rare", "set_id": set_id,
...
func _make_chest(armor: float, set_id: String = "", affixes: Array = []) -> Dictionary:
	return {
		"id": "test_chest", "slot": "chest", "display_name": "测试胸甲",
		"rarity": "epic", "set_id": set_id,
...
func _make_legs(armor: float, set_id: String = "", affixes: Array = []) -> Dictionary:
	return {
		"id": "test_legs", "slot": "legs", "display_name": "测试腿甲",
		"rarity": "rare", "set_id": set_id,
...
func _make_boots(armor: float, set_id: String = "", affixes: Array = []) -> Dictionary:
	return {
		"id": "test_boots", "slot": "boots", "display_name": "测试靴子",
		"rarity": "rare", "set_id": set_id,
...
func test_unequip_item() -> bool:
	...
	if _equipped["weapon"] != null:
		print("  FAIL: weapon slot should be null after unequip, got: %s" % str(_equipped["weapon"]))
		return false
	return true
...
func test_set_bonus_2pieces() -> bool:
	...
	if _active_sets.get("set_bone", 0) != 2:
		print("  FAIL: active_sets['set_bone'] should be 2, got: %d" % _active_sets.get("set_bone", 0))
		return false

	var bonuses = _mock_get_set_bonuses()
	if not bonuses.has("lifesteal"):
		print("  FAIL: 2-piece set bonus should contain 'lifesteal', got keys: %s" % str(bonuses.keys()))
		return false
...
func test_set_bonus_4pieces() -> bool:
	...
	if _active_sets.get("set_bone", 0) != 4:
		print("  FAIL: active_sets['set_bone'] should be 4, got: %d" % _active_sets.get("set_bone", 0))
		return false

	var bonuses = _mock_get_set_bonuses()
	# 2件阶梯仍应激活
	if not bonuses.has("lifesteal"):
		print("  FAIL: 2-piece lifesteal should still be active at 4 pieces, got keys: %s" % str(bonuses.keys()))
		return false
	# 4件阶梯加成
	...
	if not bonuses.has("crit_chance"):
		print("  FAIL: 4-piece bonus should contain 'crit_chance', got keys: %s" % str(bonuses.keys()))
		return false
	if not bonuses.has("effect_bone_explosion"):
		print("  FAIL: 4-piece effect 'bone_explosion' should be active, got keys: %s" % str(bonuses.keys()))
		return false
	return true
...
func test_affix_freeze_keys() -> bool:
	...
	if not effects.has("freeze_chance"):
		print("  FAIL: combat_effects should contain 'freeze_chance', got keys: %s" % str(effects.keys()))
		return false
	if not effects.has("freeze_duration"):
		print("  FAIL: combat_effects should contain 'freeze_duration', got keys: %s" % str(effects.keys()))
		return false
...
func test_affix_mult_stat() -> bool:
	...
	if not effects.has("damage_mult"):
		print("  FAIL: combat_effects should contain 'damage_mult', got keys: %s" % str(effects.keys()))
		return false
