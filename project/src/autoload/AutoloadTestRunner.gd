extends Node
## AutoloadTestRunner.gd - 验收执行器
## 注意：本文件被 Godot 编辑器缓存反复复活并注册为 autoload。
## 为避免它在正常启动/test_runner 中擅自 quit 整个进程，
## 默认 _ready 不做任何事；仅当命令行带 --test-acceptance 时才运行验收。
## 运行验收：godot --headless --path . --test-acceptance
